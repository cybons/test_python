...
class AdminUsersProcessor:
    ...
    def process_admin_users(self) -> pd.DataFrame:
        ...
        """
        # 社外送受信権限を持つユーザーを抽出
        email_users = self.userapp_df[self.userapp_df["application_code"] == "EMAIL01"][
            "user_code"
        ].unique()
        ...
        # ユーザー基本情報の作成
        admin_users = self.users_df.merge(self.location_df, on="user_code", how="left")

        # 社外メール権限フラグの追加
        admin_users["has_external_email"] = admin_users["user_code"].isin(email_users)

        # 組織情報の結合
        admin_users = admin_users.merge(primary_orgs, on="user_code", how="left")

        # 組織マスタとの結合
        admin_users = admin_users.merge(self.org_df, on="org_code", how="left")

        # 役職情報の結合
        admin_users = admin_users.merge(self.title_df, on="title_code", how="left")
    ...
    def validate_data(self, admin_users: pd.DataFrame) -> list[str]:
        ...
        # 必須項目のチェック
        required_columns = ["user_code", "user_name", "org_code", "org_name"]
        ...
        if email_users["location_name"].isna().any():
            null_locations = email_users[email_users["location_name"].isna()][
                "user_code"
            ].tolist()
            errors.append(
    ...
    def export_to_excel(self, admin_users: pd.DataFrame, output_path: str) -> None:
        """
        ...
        admin_users.to_excel(writer, sheet_name="admin_users", index=False)
        writer.save()
