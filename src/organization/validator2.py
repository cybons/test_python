        ...
        try:
            ...
            for sheet_name, rules in self.validation_rules.items():
                ...
                for column, master_info in rules.get("master_validation", {}).items():
                    ...
                    invalid_values = set(df[column]) - set(
                        master_df[master_info["master_column"]]
                    )
                    if invalid_values:
                        errors.append(
                            ValidationError(
...
class ApplicationFormValidator:
    def __init__(
        ...
        self.file_existence_validator = FileExistenceValidator(
            file_path, required_sheets
        )
        self.header_validator = HeaderValidator(file_path, sheet_headers)
        self.content_validator = ContentValidator(
            file_path, master_data, validation_rules
        )
...
def validate_application_form(file_path: str):
    ...
    validation_rules = {
        "配信申請": {
            "master_validation": {
                "組織コード": {
                    "master_name": "organizations",
                    "master_column": "org_code",
                },
                "雇用形態": {
                    "master_name": "employment_types",
                    "master_column": "type_code",
                },
            }
        }
    }
    ...
    validator = ApplicationFormValidator(
        Path(file_path), required_sheets, sheet_headers, master_data, validation_rules
    )
